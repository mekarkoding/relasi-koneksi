...
/**
 * Slug input that auto-generates from the configured source field
 * (`options.source`, e.g. `title_id` or `name_id`) after 2s of no typing.
...
export function AutoSlugInput(props: SlugInputProps) {
  ...
  const sourceField =
    typeof schemaType.options?.source === 'string' ? schemaType.options.source : 'title_id'
  const sourceValue = useFormValue([sourceField]) as string | undefined
  const maxLength = schemaType.options?.maxLength ?? 96

  const [autoGenerate, setAutoGenerate] = useState(
    () => !isCustomSlug(sourceValue, value?.current, maxLength),
  )
  ...
  // Debounced auto-generate from the Indonesian source field
  useEffect(() => {
  ...
  }, [sourceValue, autoGenerate, readOnly, maxLength])
