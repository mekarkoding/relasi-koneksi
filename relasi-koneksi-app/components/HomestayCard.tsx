 ...
 */
export function HomestayCard({ homestay, arch = false }: Props) {
  ...
  const whatsappButton = (
    <a
      ...
      className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-marigold px-4 py-2.5 text-sm font-semibold text-forest transition-all duration-300 ease-in-out hover:bg-marigold-dark"
    >
    ...
    return (
      <article className="group">
        ...
        <div className="px-2 pt-5 text-center">
          ...
          <p className="mt-1 text-sm font-semibold text-tamblingan">{homestay.priceRange}</p>
  ...
  return (
    <article className="group overflow-hidden rounded-2xl bg-white shadow-sm transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-lg">
      ...
      <div className="p-5">
        ...
        <p className="mt-3 text-sm font-semibold text-tamblingan">{homestay.priceRange}</p>

        <ul className="mt-3 flex flex-wrap gap-1.5" aria-label={t("facilities")}>
          {homestay.facilities.map((facility) => (
            <li
              key={facility}
              className="rounded-full bg-mist-dark px-2.5 py-0.5 text-xs text-forest/70"
            >
              {facility}
            </li>
          ))}
