...
import type { ArticlePreview, ArticleDetailType } from "@/lib/sanity/types";
import { urlForImage } from "@/lib/sanity/image";
import { pickLocale } from "@/lib/locale-content";

interface Props {
  article: ArticlePreview;
  /** Route segment for the detail link, e.g. "berita" | "sejarah" | "partnership" */
  type: ArticleDetailType;
  /** Fixed badge label for types without a category (sejarah, partnership) */
  fixedLabel?: string;
}

export function ArticleCard({ article, type, fixedLabel }: Props) {
  ...
  return (
    <article className="group overflow-hidden rounded-2xl bg-white shadow-sm transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-lg">
      <Link href={`/articles/${type}/${article.slug}`} className="block">
